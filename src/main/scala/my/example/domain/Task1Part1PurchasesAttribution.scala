...
object Task1Part1PurchasesAttribution {
  def calculate(mobAppClickstream: Dataset[MobAppClickstream], purchases: Dataset[Purchase])
    ...
    spark.sql(
      """
        ...
        |,sessions_marked AS (
        |  SELECT clickstream.*
        |  ,MAX(rn_app_open_event.app_open_rn) OVER (PARTITION BY userId ORDER BY eventTime RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) sessionRn
        |  FROM clickstream
